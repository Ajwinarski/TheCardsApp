...
class War: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        myCardsPos = CGPoint(x: self.size.width - self.size.width/8,
                             y: self.size.height - self.size.height/8)
        dealerCardsPos = CGPoint(x: self.size.width - self.size.width/8,
                                 y: self.size.height + self.size.height/8)
    ...
    func setupDeck() {
        for item in 0...51 {
            ...
            if item % 2 == 0 {
                myCards.addCardToFront(card: card)
                let node: SKNode = myCards.getFirst()
                if myCards.getFirst().getValue() == 1 {
                    myCards.getFirst().setValue(14)
                }
                node.position = myCardsPos
                node.setScale(0.6)
                ...
                if dealerCards.getFirst().getValue() == 1 {
                    dealerCards.getFirst().setValue(14)
                }
    ...
    func updateCardsOnBoard() {
        myCards.getFirst().isHidden = false
        dealerCards.getFirst().isHidden = false
    }
    /*
